...
describe('Create table', () => {
    ...
    it('TTLSettings and uniformPartitions', async () => {
        ...
        await driver.tableClient.withSession(async (session) => {
            ...
            expect(createdTableDescription.tableStats?.partitions).toStrictEqual(new long_1.default(10, undefined, true));
        });
    });
    ...
    it('partitioningSettings', async () => {
        ...
        await driver.tableClient.withSession(async (session) => {
            ...
            expect(createdTableDescription.tableStats?.partitions).toStrictEqual(new long_1.default(50, undefined, true));
        });
    });
    ...
    it('partitionAtKeys and readReplicasSettings', async () => {
        ...
        await driver.tableClient.withSession(async (session) => {
            await session.createTable(tableName, tableDescription);
            const createdTableDescription = await session.describeTable(tableName, new table_1.DescribeTableSettings().withIncludeTableStats(true));
            expect(createdTableDescription.tableStats?.partitions).toStrictEqual(new long_1.default(8, undefined, true));
        });
    });
});
