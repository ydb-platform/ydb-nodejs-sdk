...
export class Session extends EventEmitter implements ICreateSessionResult {
    ...
    constructor(
        ...
        super();
        ContextWithLogger.get('ydb-nodejs-sdk:Session.constructor', this.logger);
    }
    ...
    public async rollbackTransaction(txControl: IExistingTransaction, settings?: RollbackTransactionSettings): Promise<void> {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:Session.rollbackTransaction', this);
    ...
    public async executeQuery(
        ...
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:Session.executeQuery', this);
    ...
    public async streamReadTable(
        ...
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:Session.streamReadTable', this);
    ...
    public async streamExecuteScanQuery(
        ...
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:Session.streamExecuteScanQuery', this);
        let queryToExecute: IQuery;
    ...
    private executeStreamRequest<Req, Resp extends PartialResponse<IRes>, IRes, Res>(
        ...
        return new Promise((resolve, reject) => {
            /* ctx-off */ apiStreamMethod(request, (error, response) => {
                try {
                    if (error) {
                        if (error instanceof StreamEnd) {
                            /* ctx-off */ resolve();
                        } else {
                            /* ctx-off */ reject(error);
                        }
                    } else if (response) {
                        const operation = {
                        ...
                        /* ctx-off */ YdbError.checkStatus(operation);

                        if (!response.result) {
                            /* ctx-off */ reject(new MissingValue('Missing result value!'));

                            return;
                        }

                        const result = /* ctx-off */ transformer(response.result!);

                        ctx.doSync(() => consumer(result));
                    }
                } catch (error_) {
                    /* ctx-off */ reject(error_);
                }
            });
        });
    }
...
export class SessionPool extends EventEmitter {
    ...
    constructor(settings: ITableClientSettings) {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:SessionPool.constructor', settings.logger);
    ...
    private initListeners(keepAlivePeriod: number) {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:SessionPool.initListeners', this);

        return setInterval(async () => Promise.all(
            /* ctx-off */ _.map([...this.sessions], (session: Session) => /* ctx-off */ session.keepAlive()
    ...
    private async createSession(): Promise<Session> {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:SessionPool.createSession', this);
    ...
    private deleteSession(session: Session): Promise<void> {
        ...
        if (this.waiters.length > 0) {
            /* ctx-off */ this.acquire().then((session) => {
    ...
    private acquire(timeout = 0): Promise<Session> {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:SessionPool.acquire', this);

        for (const session of this.sessions) {
            if (/* ctx-off */ session.isFree()) {
        ...
        if (this.sessions.size + this.newSessionsRequested - this.sessionsBeingDeleted <= this.maxLimit) {
            ...
            return /* ctx-off */ this.createSession()
        ...
        return new Promise((resolve, reject) => {
            ...
            const waiter = (session: Session) => {
                /* ctx-off */ clearTimeout(timeoutId);
                /* ctx-off */ resolve(ctx.doSync(() => session.acquire()));
            };

            if (timeout) {
                timeoutId = setTimeout(() => {
                    /* ctx-off */ this.waiters.splice(/* ctx-off */ this.waiters.indexOf(waiter), 1);
                    /* ctx-off */ reject(
    ...
    private async _withSession<T>(session: Session, callback: SessionCallback<T>, maxRetries = 0): Promise<T> {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:SessionPool._withSession', this);
    ...
    public async withSession<T>(callback: SessionCallback<T>, timeout = 0): Promise<T> {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:SessionPool.withSession', this);
    ...
    public async withSessionRetry<T>(callback: SessionCallback<T>, timeout = 0, maxRetries = 10): Promise<T> {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:SessionPool.withSessionRetry', this);
        const session = await ctx.do(() => this.acquire(timeout));
...
export class TableClient extends EventEmitter {
    ...
    public async withSession<T>(callback: (session: Session) => Promise<T>, timeout = 0): Promise<T> {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:TableClient.withSession', this);
    ...
    public async withSessionRetry<T>(callback: (session: Session) => Promise<T>, timeout = 0, maxRetries = 10): Promise<T> {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:TableClient.withSessionRetry', this);

        return ctx.doSync(() => this.pool.withSessionRetry(callback, timeout, maxRetries));
    }

    public async destroy() {
        const ctx = ContextWithLogger.get('ydb-nodejs-sdk:TableClient.destroy', this);
...
export class AlterTableDescription {
    ...
    constructor() {
        ContextWithLogger.get('ydb-nodejs-sdk:AlterTableDescription.constructor', '<logger>');
    }
    ...
    withSetTtl(columnName: string, expireAfterSeconds = 0) {
        ContextWithLogger.get('ydb-nodejs-sdk:AlterTableDescription.withSetTtl', this);
    ...
    withDropTtl() {
        ContextWithLogger.get('ydb-nodejs-sdk:AlterTableDescription.withDropTtl', this);
