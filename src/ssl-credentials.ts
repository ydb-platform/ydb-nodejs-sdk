...
const makeInternalRootCertificates = () => {
    ...
    systemRootCertificates = nodeRootCertificates && nodeRootCertificates.length > 0 ? Buffer.from(nodeRootCertificates.join('\n')) : fallbackSystemRootCertificates;

    return Buffer.concat([internalRootCertificates, systemRootCertificates]);
};

export const makeDefaultSslCredentials = () => {
...
export const makeSslCredentials = (endpoint: string, logger: Logger, sslCredentials: ISslCredentials | undefined): ISslCredentials | undefined => {
