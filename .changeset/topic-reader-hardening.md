...
- `onCommittedOffset` fires on every server-confirmed advance — commit acks and stop-request watermarks — including ended and stopped partitions. A `commitOffset` returned by the start hook remains optimistic until the server reports it back, so it neither resolves pending commits nor notifies the observer prematurely.
