...
emits `partition.committed{session}` so the observer sees every durable advance
(commit acks and stop watermarks). A start-hook `commitOffset` only raises the
current grant's `commitRangeFloor`: it filters outgoing ranges and advances delivery
stitching, but stored pending ranges remain recoverable for the next grant and no
waiter or observer is notified until the server confirms it.
...
| ready                     | `reader.commit`                                                                                     | same                    | recordCommit: retain ranges above the confirmed watermark, subtract claimed coverage (an overlap on the wire is session-fatal), and filter this grant's send at `commitRangeFloor`; ranges already below the confirmed watermark resolve, while fully claimed/optimistically covered ranges remain pending; uncovered remainder on a `stopped` entry → immediate `commit.rejected` (the commit lost the race with the partition stop — never parked on the gc); else buffer pending + claim; `send.commit` only if the session is live on the current stream AND entry is `active`/`stopping-graceful`/`ended` AND `!ackPending`; `commit.rejected` when no entry exists |
| ready                     | `reader.partition.start_ready`                                                                      | same                    | ackPartitionStart: no-op if grantId stale / not ackPending / not active / not indexed; else `ackPending=false`, `commitOffset` override raises the optimistic `commitRangeFloor` without resolving pending commits, effects `send.start_response`, clear `partition_reassign_gc:key`, re-send each pending's exact remaining ranges above this grant's floor (the single send)                                          |
